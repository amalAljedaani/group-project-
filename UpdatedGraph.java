...
public class Graph {
    ...
    public void makeGraph(int vertexNum, int edgeNum){
        ...
        isDigrap= true; // By default, assume the graph is directed
    ...
    public List<Edge> getEdges() {
       ...
       /* Edge e = null;
        for (int i = 0; i < verticess.size(); i++) {
            for (int j = 0; j < verticess.get(i).adjLists.size(); j++) {
                e=verticess.get(i).adjLists.get(j);
            }
            if(!edges.get(i).source.getLabel().matches(e.destination.Label)&&!edges.get(i).source.getLabel().matches(e.source.Label))
                edges.add(e);
        }
        return edges;
        */
    ...
    public Edge createEdge(Vertex source, Vertex dest, int weight){
